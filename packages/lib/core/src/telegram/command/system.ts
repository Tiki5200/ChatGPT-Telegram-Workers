...
export class ImgCommandHandler implements CommandHandler {
    ...
    handle = async (
        ...
        const sender = MessageSender.fromMessage(
        ...
        if (subcommand === '') {
            ...
            const text
                = `${ENV.I18N.command.help.img}\n\n`
                    + `${imgAgent?.name || 'Nan'} | `
                    + `${imgAgent?.model(context.USER_CONFIG) || 'Nan'}\n\n`
                    + '用法：\n'
                    + '/img human 图片描述\n'
                    + '/img cat 图片描述';
        ...
        try {
            ...
            if (!agent) {
                return sender.sendPlainText(
                    'ERROR: Image generator not found',
                );
            }

            const rawPrompt = subcommand.trim();
            const lowerPrompt = rawPrompt.toLowerCase();

            const isCat
                = lowerPrompt === 'cat'
                    || lowerPrompt.startsWith('cat ');

            const isHuman
                = lowerPrompt === 'human'
                    || lowerPrompt.startsWith('human ');

            const mode: 'human' | 'cat'
                = isCat ? 'cat' : 'human';

            const userPrompt = isCat
                ? rawPrompt.slice(3).trim()
                : isHuman
                    ? rawPrompt.slice(5).trim()
                    : rawPrompt;
            ...
            const referenceUrl = mode === 'cat'
                ? ENV.DIANA_CAT_REFERENCE_URL
                : ENV.DIANA_HUMAN_REFERENCE_URL;

            if (!referenceUrl) {
                const variableName = mode === 'cat'
                    ? 'DIANA_CAT_REFERENCE_URL'
                    : 'DIANA_HUMAN_REFERENCE_URL';

                return sender.sendPlainText(
                    `ERROR: ${variableName} is missing`,
                );
            }

            const identityPrompt = mode === 'cat'
                ? [
                        'Create a new image featuring the exact same cat character',
                        'shown in the supplied reference image.',
                        'Preserve the cat’s identity, face, fur pattern,',
                        'body shape, and recognizable features.',
                        `New scene: ${userPrompt}`,
                    ].join(' ')
                : [
                        'Create a new image featuring the exact same woman',
                        'shown in the supplied reference image.',
                        'Preserve her identity, facial features, hairstyle,',
            ...
            if (!resp.ok) {
                return sender.sendPlainText(
                    `ERROR: ${resp.statusText} ${await resp.text()}`,
                );
            }
